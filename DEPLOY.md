...
4. Set **`MACRO_REFRESH_SECRET`** to a long random string (same value you type on the home page under **Refresh macro data**).
5. Default **`npm run build`** runs **`prisma migrate deploy`** then **`next build`**, so tables are created on each deploy when `DATABASE_URL` is set.
...
5. After deploy, open the live site → **Refresh macro data** (password = `MACRO_REFRESH_SECRET`), or run `npm run macro:refresh` locally with prod `DATABASE_URL`.
